...
for time_idx=1:end_time
    ...
    for num_iter=1:end_iter
        ...
        if N_grouping~=0
            ...
            % freq_vector = - size(V_complete, 1)/2+2:N_grouping:size(V_complete, 1)/2;
        ...
        if index_bf~=2
            ...
            [~, angbit, Nst] = bfCompressQuantizeBit(V(:,1:NumSTSVec,:),NumBitsPhi,NumBitsPsi);
            V_tx = reshape(angbit, [], 1);
        ...
        %% TX feedback
        cfgHEFeedb = cfgHEBase;
        cfgHEFeedb.NumSpaceTimeStreams = NumTxAnts; 
        cfgHEFeedb.SpatialMapping = 'Direct';  % Each TxAnt carries a STS
        cfgHEFeedb.MCS = 2; 
        psduLength = getPSDULength(cfgHEFeedb);
        ind_feedback = wlanFieldIndices(cfgHEFeedb);
        numPkts = ceil(size(V_tx, 1)/psduLength/8);
        
        rxDataBits = zeros(psduLength*numPkts*8, 1);
        for pktidx = 1:numPkts
            idx_start = (pktidx-1)*psduLength*8 + 1;
            idx_end = min(idx_start + psduLength*8, size(V_tx, 1));
            txFeedbackSig = wlanWaveformGenerator(V_tx(idx_start:idx_end, :), cfgHEFeedb);
    
            txPadFeed = [txFeedbackSig; zeros(numPadZeros,cfgHEFeedb.NumTransmitAntennas)];
            
            % Pass through a fading indoor TGax channel
            reset(tgaxChannel); % Reset channel for different realization
    
            % Sound the emulated channel
            chanOutFeedback = tgaxChannel(txPadFeed); 
    
            % Add AWGN
            rxFeedbackSig = awgn(chanOutFeedback, packetSNR);
    
            %% Feedback Recovery Per User
            % Packet detect and determine coarse packet offset
            coarsePktOffset = wlanPacketDetect(rxFeedbackSig,chanBW);
            if isempty(coarsePktOffset) % If empty no L-STF detected; packet error
                continue; % Go to next loop iteration
            end
        
            % Extract L-STF and perform coarse frequency offset correction
            lstf = rxFeedbackSig(coarsePktOffset+(ind_feedback.LSTF(1):ind_feedback.LSTF(2)),:);
            coarseFreqOff = wlanCoarseCFOEstimate(lstf,chanBW);
            rxFeedbackSig = frequencyOffset(rxFeedbackSig,fs,-coarseFreqOff);
        
            % Extract the non-HT fields and determine fine packet offset
            nonhtfields = rxFeedbackSig(coarsePktOffset+(ind_feedback.LSTF(1):ind_feedback.LSIG(2)),:);
            ...
            rxLLTF = rxFeedbackSig(pktOffset+(ind_feedback.LLTF(1):ind_feedback.LLTF(2)),:);
            fineFreqOff = wlanFineCFOEstimate(rxLLTF,chanBW);
            rxFeedbackSig = frequencyOffset(rxFeedbackSig,fs,-fineFreqOff);
        
            % HE-LTF demodulation and channel estimation
            rxHELTF = rxFeedbackSig(pktOffset+(ind_feedback.HELTF(1):ind_feedback.HELTF(2)),:);
            heltfDemod = wlanHEDemodulate(rxHELTF,'HE-LTF',cfgHEFeedb);
            [chanEst,pilotEst] = wlanHELTFChannelEstimate(heltfDemod,cfgHEFeedb);
        
            % Data demodulate
            rxData = rxFeedbackSig(pktOffset+(ind_feedback.HEData(1):ind_feedback.HEData(2)),:);
            demodSym = wlanHEDemodulate(rxData,'HE-Data',cfgHEFeedb);
        
            % Pilot phase tracking
            % Average single-stream pilot estimates over symbols (2nd dimension)
            pilotEstTrack = mean(pilotEst,2);
            demodSym = wlanHETrackPilotError(demodSym,pilotEstTrack,cfgHEFeedb,'HE-Data');
        
            % Estimate noise power in HE fields
            nVarEst = wlanHEDataNoiseEstimate(demodSym(ofdmInfo.PilotIndices,:,:),pilotEstTrack,cfgHEFeedb);
            ...
            [eqDataSym,csi] = wlanHEEqualize(demodDataSym,chanEstData,nVarEst,cfgHEFeedb,'HE-Data');
        
            % Recover data
            rxDataBits(idx_start:idx_start+psduLength*8-1) = wlanHEDataBitRecover(eqDataSym,nVarEst,csi,cfgHEFeedb,'LDPCDecodingMethod','norm-min-sum');
        end

        %% Obtain angles back
        if index_bf~=2
            V_rec = bfDecompressBit(rxDataBits(1:size(V_tx, 1)),Nst,Nr,Nc,NumBitsPhi,NumBitsPsi); % Nst-by-Nsts-by-Nt
        else
            V_rec = V;
        end
