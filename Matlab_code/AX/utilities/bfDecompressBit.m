function Vtilda = bfDecompressBit(angbit,Nst,Nr,Nc,bphi,bpsi)
...
p = min([Nc,Nr-1]);
NumBits = size(angbit, 1);
NumAngles = NumBits/Nst/((bphi+bpsi)/2);
...
for ii = Nr-1:-1:max(Nr-Nc,1)
    for jj = 1:ii
        angbittmp = angbit(bitcnt:bitcnt+bphi-1);
        angidxtmp = bit2int(angbittmp, bphi);
        angles(angcnt,1,:) = (2*angidxtmp+1)*pi/(2^bphi);
        angcnt = angcnt + 1;
        bitcnt = bitcnt + bphi;
    end
    
    for jj = 1:ii
        angbittmp = angbit(bitcnt:bitcnt+bpsi-1);
        angidxtmp = bit2int(angbittmp, bpsi);
        angles(angcnt,1,:) = (2*angidxtmp+1)*pi/(2^(bpsi+2));
        angcnt = angcnt + 1;
        bitcnt = bitcnt + bpsi;
    end
end
