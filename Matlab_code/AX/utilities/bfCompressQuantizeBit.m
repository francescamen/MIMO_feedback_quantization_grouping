function [angidx, angbit, Nst] = bfCompressQuantizeBit(VTilda,bphi,bpsi)
...
angbit = zeros(Nst,NumAngles*(bphi+bpsi)/2);
angcnt = 1;
bitcnt = 1;
for ii = Nr-1:-1:max(Nr-Nc,1)
    for jj = 1:ii
        angidxtmp = round(0.5*((angles(angcnt,1,:)*((2^bphi))/pi)-1));
        angidx(:,angcnt) = angidxtmp;
        angcnt = angcnt + 1;
        angbittmp = int2bit(angidxtmp, bphi);
        angbit(:, bitcnt:bitcnt+bphi-1) = squeeze(angbittmp).';
        bitcnt = bitcnt + bphi;
    end
    
    for jj = 1:ii
        angidxtmp = round(0.5*((angles(angcnt,1,:)*(2^(bpsi+2))/pi)-1));
        angidx(:,angcnt) = angidxtmp;
        angcnt = angcnt + 1;
        angbittmp = int2bit(angidxtmp, bpsi);
        angbit(:, bitcnt:bitcnt+bpsi-1) = squeeze(angbittmp).';
        bitcnt = bitcnt + bpsi;
    end
end
end