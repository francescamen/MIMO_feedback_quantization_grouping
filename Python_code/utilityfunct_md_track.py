...
def joint_aoa_tof_estimation(cfr_sample, aoa_matrix, toa_matrix, num_ant, num_subc, num_angles):
    ...
    # plt.figure()
    # plt.pcolormesh(power_matrix_cfr_aoa_toa)
    # plt.show()
...
def md_track_1d(cfr_sample, toa_matrix, num_subc, num_iteration_refinement, threshold):
    # start = time.time()
    paths = []
    ...
    cfr_sample_residual = cfr_sample
    while True:
        matrix_cfr_toa = np.dot(toa_matrix, cfr_sample_residual) / num_subc
        # plt.figure()
        # plt.plot(abs(matrix_cfr_toa))
        # plt.show()
        power_matrix_cfr_toa = np.power(np.abs(matrix_cfr_toa), 2)
        path_toa = np.argmax(power_matrix_cfr_toa, axis=0)

        path_amplitude = matrix_cfr_toa[path_toa]
        path_power = power_matrix_cfr_toa[path_toa]
        ...
        cfr_sample_residual = cfr_sample_residual - signal_path
        # plt.figure()
        # plt.plot(abs(cfr_sample_residual))
        # plt.plot(abs(cfr_sample_residual - signal_path))
        # plt.show()
    ...
    for iteration in range(num_iteration_refinement):
        for path_idx in range(num_estimated_paths):
            ...
            matrix_cfr_toa = np.dot(toa_matrix, cfr_single_path) / num_subc

            power_matrix_cfr_toa = np.power(np.abs(matrix_cfr_toa), 2)
            path_toa = np.argmax(power_matrix_cfr_toa)
    ...
    return paths_refined_amplitude, paths_refined_toa
...
def joint_aoa_tof_dop_estimation(cfr_window, aoa_matrix, toa_matrix, dop_matrix, num_ant, num_subc, num_pkts,
    ...
    # option 1
    # cfr_window_reshape = np.reshape(cfr_window, (-1, num_ant))
    # matrix_cfr_aoa = np.dot(cfr_window_reshape, np.transpose(np.conj(aoa_matrix)))
    # matrix_cfr_aoa_reshape = np.reshape(matrix_cfr_aoa, (num_pkts, num_subc, num_angles))
    # matrix_cfr_aoa_perm = np.moveaxis(matrix_cfr_aoa_reshape, 0, 1)
    # matrix_cfr_aoa_perm_reshape = np.reshape(matrix_cfr_aoa_perm, (num_subc, -1))
    #
    # matrix_cfr_aoa_toa = np.dot(np.transpose(np.conj(toa_matrix)), matrix_cfr_aoa_perm_reshape)
    # matrix_cfr_aoa_toa_reshape = np.reshape(matrix_cfr_aoa_toa, (num_times, num_pkts, num_angles))
    # matrix_cfr_aoa_toa_perm = np.moveaxis(matrix_cfr_aoa_toa_reshape, 0, 1)
    # matrix_cfr_aoa_toa_perm_reshape = np.reshape(matrix_cfr_aoa_toa_perm, (num_pkts, -1))
    #
    # matrix_cfr_aoa_toa_dop = np.dot(np.transpose(np.conj(dop_matrix)), matrix_cfr_aoa_toa_perm_reshape) / \
    #                          (num_ant * num_subc * num_pkts)

    # option 2
    # matrix_cfr_aoa = np.dot(cfr_window, np.transpose(np.conj(aoa_matrix)))
    #
    # matrix_cfr_aoa_toa = np.dot(np.transpose(np.conj(toa_matrix)), matrix_cfr_aoa)
    #
    # matrix_cfr_aoa_toa_dop = np.dot(np.transpose(np.conj(dop_matrix)), matrix_cfr_aoa_toa) / \
    #                          (num_ant * num_subc * num_pkts)
...
def md_track_3d(cfr_window, aoa_matrix, toa_matrix, dop_matrix, num_ant, num_subc, num_angles, num_pkts,
    ...
    for iteration in range(num_iteration_refinement):
        for path_idx in range(num_estimated_paths):
            ...
            dopl_sign = np.reshape(np.conj(dop_matrix[:, path_dop]), (1, 1, -1))
            time_aoa_signal = np.expand_dims(np.expand_dims(np.conj(aoa_matrix[:, path_aoa]), -1) * \
                                             np.repeat(np.expand_dims(np.conj(toa_matrix[path_toa, :]), 0), num_ant, axis=0),
                                             -1)
